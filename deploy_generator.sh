...
cat > user_data_locust.sh << 'EOF'
#!/bin/bash
yum update -y
yum install -y python3-pip git

# instala locust para o usuário ec2-user
su - ec2-user -c 'pip3 install --user "urllib3<2.0" locust'

# garante PATH com o binário do locust
...
class WordPressUser(HttpUser):
    ...
    @task(10)
    def view_post(self):
        self.client.get(f"/post-{random.randint(1, 100)}/", name="/post-detail")
PY_EOF
